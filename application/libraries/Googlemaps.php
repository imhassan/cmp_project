...
class Googlemaps {
	...
	function add_polygon($params = array())
	{
		...
		$polygon['infowindow_content'] = '';						// If not blank, creates an infowindow (aka bubble) with the content provided. Can be plain text or HTML
		...
		';
		if ($polygon['infowindow_content']!="") {
			
			// Escape any quotes in the event that HTML is being added to the infowindow
			$polygon['infowindow_content'] = str_replace('\"', '"', $polygon['infowindow_content']);
			$polygon['infowindow_content'] = str_replace('"', '\"', $polygon['infowindow_content']);
			
			$polygon_output .= '
			
			google.maps.event.addListener(polygon_'.count($this->polygons).', "click", function(event) {
				iw_'.$this->map_name.'.setContent("'.$polygon['infowindow_content'].'");
				iw_'.$this->map_name.'.setPosition(event.latLng);
				iw_'.$this->map_name.'.open('.$this->map_name.', this);
			';
			if ($polygon['onclick']!="") { $polygon_output .= $polygon['onclick'].'
			'; }
			$polygon_output .= '
			});
			';
	...
	function get_lat_long_from_address($address, $attempts = 0)
	{
		...
		if($this->sensor == "true") {
			$data_location .= "&sensor=".$this->sensor;
		}
